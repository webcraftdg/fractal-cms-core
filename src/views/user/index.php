...
<div class="fc-list-header">
    <div>
        <h2 class="fc-list-header-title">Liste des utilisateurs</h2>
    </div>
    <div>
        <?php
        if (Yii::$app->user->can(Constant::PERMISSION_MAIN_USER.Constant::PERMISSION_ACTION_CREATE) === true):

            echo Html::beginTag('a', ['href' => Url::to(['user/create']), 'class' => 'fc-btn fc-btn-success']);
            ?>
        ...
        <?php
            ...
            foreach ($models as $model) {
             ...
             echo Html::beginTag('div', ['class' => implode(' ', $classes), 'fractal-cms-core-list-line' => $model->id]);
                 echo Html::tag('div', ucfirst($model->firstname), ['class' => 'flex-1 font-medium']);
                 echo Html::tag('div', ucfirst($model->lastname), ['class' => 'flex-1 font-medium']);
                 echo Html::beginTag('div', ['class' => 'flex items-center gap-3 sm:w-auto']);
                     if (Yii::$app->user->can(Constant::PERMISSION_MAIN_USER.Constant::PERMISSION_ACTION_UPDATE) === true)  {
                         echo Html::beginTag('a', ['href' => Url::to(['user/update', 'id' => $model->id]), 'class' => 'text-indigo-500 hover:text-indigo-700 transition icon-link', 'title' => 'Editer']);
                         ?>
                         <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                         ...
                         echo Html::endTag('a');
                            ...
                            ?>
                                    <svg width="24px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                        <g id="svg-active-false" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                            <g id="ic-switch-off" fill-rule="nonzero" fill="#fd7e14">
                                                <path d="M3,12 C3,14.2046438 4.78852545,16 6.9947834,16 L17.0052166,16 C19.2082844,16 21,14.209139 21,12 C21,9.79535615 19.2114745,8 17.0052166,8 L6.9947834,8 C4.79171562,8 3,9.790861 3,12 Z M1,12 C1,8.68773136 3.68570682,6 6.9947834,6 L17.0052166,6 C20.3154275,6 23,8.69016475 23,12 C23,15.3122686 20.3142932,18 17.0052166,18 L6.9947834,18 C3.68457248,18 1,15.3098352 1,12 Z M17,15 C15.3431458,15 14,13.6568542 14,12 C14,10.3431458 15.3431458,9 17,9 C18.6568542,9 20,10.3431458 20,12 C20,13.6568542 18.6568542,15 17,15 Z" id="Rectangle-false-75">
                                                </path>
                                            </g>
                                        </g>
                                    </svg>
                        ...
                        if (Yii::$app->user->can(Constant::PERMISSION_MAIN_USER.Constant::PERMISSION_ACTION_DELETE) === true && Yii::$app->user->getIdentity()->getId() !== $model->id)  {
                            echo Html::beginTag('a', ['href' => Url::to(['api/user/delete', 'id' => $model->id]), 'class' => 'icon-link text-red-500 hover:text-red-700 transition user-button-delete', 'title' => 'Supprimer']);
                            ?>
                            <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
