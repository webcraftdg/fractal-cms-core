...
<div class="fc-list-header">
    <div>
        <h2 class="fc-list-header-title">Liste des paramètres</h2>
    </div>
    <div>
        <?php
        if (Yii::$app->user->can(Constant::PERMISSION_MAIN_PARAMETER.Constant::PERMISSION_ACTION_CREATE) === true):

            echo Html::beginTag('a', ['href' => Url::to(['parameter/create']), 'class' => 'fc-btn fc-btn-success']);
            ?>
        ...
        <?php
            ...
            foreach ($modelsQuery->each() as $model) {
             ...
             echo Html::beginTag('div', ['class' => implode(' ', $classes), 'fractal-cms-core-list-line' => $model->id]);
                ...
                echo Html::tag('div', ucfirst($model->group), ['class' => implode(' ', $className)]);
                echo Html::tag('div', ucfirst($model->name), ['class' => implode(' ', $className)]);
                echo Html::tag('div', ucfirst($model->value), ['class' => implode(' ', $className)]);
                 echo Html::beginTag('div', ['class' => 'flex items-center gap-3 sm:w-auto']);
                     if (Yii::$app->user->can(Constant::PERMISSION_MAIN_PARAMETER.Constant::PERMISSION_ACTION_UPDATE) === true)  {
                         echo Html::beginTag('a', ['href' => Url::to(['parameter/update', 'id' => $model->id]), 'class' => 'text-indigo-500 hover:text-indigo-700 transition icon-link', 'title' => 'Editer']);
                         ?>
                         <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        if (Yii::$app->user->can(Constant::PERMISSION_MAIN_PARAMETER.Constant::PERMISSION_ACTION_DELETE) === true)  {
                            echo Html::beginTag('a', ['href' => Url::to(['api/parameter/delete', 'id' => $model->id]), 'class' => 'icon-link text-red-500 hover:text-red-700 transition user-button-delete', 'title' => 'Supprimer']);
                            ?>
                            <svg width="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
